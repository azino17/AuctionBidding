...
// Function to search for a node with a specific item ID in AVL tree
...
void AddItem(HWND hwnd) {
    ...
    // Append the input values to the file with serial number
...
void updateBid(AVLNode* root, char itemId[], char currentBid[], char newBid[]) {
    ...
    if (compare == 0) {
        ...
        if (newBidValue > currentBidValue) {
            // Update current bid in the AVL tree node
            sprintf(root->currentBid, "%d", newBidValue); // Update current bid

            // Display bid placed successfully message
...
// Update the current bid displayed in the user interface
